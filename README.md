...
下载 [DMG](https://github.com/EricZzzzz221b/token-usage/releases/download/v1.2.0-beta.14/TokenUsage_1.2.0-beta.14_arm64.dmg) 后，将 `Token用量.app` 拖入 `Applications`。当前包采用 ad-hoc 签名；首次启动请在 Finder 中右键应用并选择“打开”。beta.14 新增账号模式识别、Credits 与重置机会、Codex 实时任务状态、完成通知和会话跳转，并通过增量读取会话事件降低后台扫描开销。

这是 macOS 预览版。安装前建议退出旧版；遇到任务状态或额度显示异常时，请在 [Issues](https://github.com/EricZzzzz221b/token-usage/issues) 中附上应用版本和复现步骤，但不要上传 `auth.json` 或完整 Codex 会话日志。
